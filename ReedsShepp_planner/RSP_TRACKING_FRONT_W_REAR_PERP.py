"""
COMMENTS
This code takes perp from the rear point
"""
...
def calc_perp(x,y,theta,pt1,pt2):
    """
    find the rear point here
    """
    x = x - 2.33*m.cos(theta)
    y = y - 2.33*m.sin(theta)
...
def calc_target(x,y,theta,goal_points):
    ...
    return (best_target,proximity,skip,[xp,yp])
...
def seek_one_pid(start,goal,true_goal):
    ...
    if heading_error > m.pi/2:
        heading_error = 0.1*(m.pi - heading_error)
        signal = True
    if heading_error < -m.pi/2:
        heading_error = -0.1*(m.pi - abs(heading_error))
        signal = True
    print("heading_error: {}".format(heading_error))
    s = Kp*heading_error
...
def seek_one_pure_pursuit(start,goal,true_goal,perp):
    ...
    #also need to find the rear
    x_rear = xs - 2.33*m.cos(thetas)
    y_rear = ys - 2.33*m.sin(thetas)
    ...
    LATERAL CONTROL
    """
    ld = m.sqrt((x_rear - xt)**2 + (y_rear - yt)**2) #this is the lookahead distance
    #need to find the angle alpha
    beta = m.atan2((yt-y_rear),(xt - x_rear))
    alpha = wrapToPi(beta - thetas)
    s = m.atan2((2*2.33*m.sin(alpha)), (ld))
    ...
    xs,ys,thetas = update(xs,ys,thetas,v,s)
...
def path_track(path):
    ...
    while len(dummy_gp) >1:
        ...
        x,y,theta,s = seek_one_pure_pursuit([x,y,theta],[xt,yt],goal, perp)
        # print(m.degrees(s))
        plt.pause(0.0001)
...
def path_track3(path,thetas):
    ...
    tic = time.time()
    xstart, ystart = path[0]
    ##shifting the start point (BAD  _IDEA_)
    # xstart = xstart + 2.33*m.cos(thetas)
    # ystart = ystart + 2.33*m.sin(thetas)
...
#         x,y,theta,s = seek_one_pure_pursuit([x,y,theta],[xg,yg])
#         plt.pause(0.0001)
skip = False
while len(dummy_gp) >1:
    #first step would be to find the target point
